<template>
  <div class="agent-config-wrapper">
    ...
    <!-- 已上传文件展示 -->
<div v-if="selectedFile && !imagePreview" class="file-info-bar">
  <div class="file-bar-left">
    <i :class="getFileIcon(selectedFile.name)" :style="{ color: getFileIconColor(selectedFile.name) }"></i>
    <span class="file-bar-name">{{ selectedFile.name }}</span>
  </div>
  <div class="file-bar-right">
    <span class="file-bar-size">{{ formatFileSize(selectedFile.size) }}</span>
    <i class="fas fa-times file-bar-del" @click="clearSelectedFile"></i>
  </div>
</div>
