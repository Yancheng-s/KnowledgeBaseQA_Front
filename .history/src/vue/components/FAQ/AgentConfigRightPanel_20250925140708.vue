  ...
  <div class="agent-config-wrapper">
    ...
    <div v-if="uploadedFiles.length > 0" class="uploaded-files-container mb-4">
      <div class="flex flex-wrap gap-2">
        <div v-for="(file, index) in uploadedFiles" :key="index" class="file-item">
          ...
          <!-- 非图片文件显示卡片 -->
          <div v-else class="flex items-center gap-2 px-3 py-2 bg-gray-50 rounded-lg border border-gray-200 min-w-0">
            <!-- 文件图标 -->
            ...
            <div class="flex-1 min-w-0">
              <div class="font-medium text-sm text-gray-900 truncate max-w-[180px]">{{ file.name }}</div>
            ...
            <button 
              @click="removeFile(index)" 
              class="flex-shrink-0 w-4 h-4 bg-gray-200 hover:bg-red-100 text-gray-600 hover:text-red-500 rounded-full flex items-center justify-center text-xs transition-colors"
            >
              ×
            </button>
          </div>
    ...
    <div class="input-container">
      <div class="relative rounded-lg p-3 min-h-[120px] border border-gray-200">
        <textarea
          v-model="inputText"
          class="w-full bg-transparent border-none outline-none text-gray-700 text-sm mb-4 resize-none"
          placeholder="请输入内容..."
          rows="3"
        ></textarea>
