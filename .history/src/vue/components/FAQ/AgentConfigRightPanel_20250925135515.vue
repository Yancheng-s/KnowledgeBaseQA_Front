        ...
        <div v-for="(file, index) in uploadedFiles" :key="index" class="file-item">
          ...
          <div v-if="isImageFile(file.name)" class="relative">
            ...
            <button 
              ...
              class="absolute -top-1 -right-1 w-5 h-5 bg-gray-600 hover:bg-red-500 text-white rounded-full flex items-center justify-center text-xs transition-colors"
            >
              ×
            </button>
          </div>
          ...
          <!-- 非图片文件显示卡片 -->
          <div v-else class="flex items-center gap-3 px-4 py-3 bg-white rounded-lg border border-gray-200 shadow-sm hover:shadow-md transition-shadow min-w-0 max-w-[280px]">
            <!-- 文件图标 -->
            <div class="flex-shrink-0">
              <i :class="[getFileIcon(file.name), 'text-2xl']" :style="{ color: getFileIconColor(file.name) }"></i>
            </div>
            ...
            <button 
              @click="removeFile(index)" 
              class="flex-shrink-0 w-6 h-6 bg-gray-100 hover:bg-red-100 text-gray-400 hover:text-red-500 rounded-full flex items-center justify-center text-sm transition-colors"
            >
              ×
            </button>
          </div>
