        ...
        <div v-for="(file, index) in uploadedFiles" :key="index" class="file-item">
          ...
          <div v-if="isImageFile(file.name)" class="relative">
            ...
            <button 
              ...
              class="absolute -top-2 -right-2 w-5 h-5 bg-gray-600 hover:bg-red-500 text-white rounded-full flex items-center justify-center text-xs transition-colors"
            >
              ×
            </button>
          </div>
          ...
          <!-- 非图片文件显示卡片 -->
          <div v-else class="flex items-center gap-2 px-3 py-2 bg-white rounded-lg border border-gray-200 shadow-sm hover:shadow-md transition-shadow min-w-0">
            <!-- 文件图标 -->
            ...
            <div class="flex-1 min-w-0">
              <div class="font-medium text-xs text-gray-900 truncate max-w-[120px]">{{ file.name }}</div>
            ...
            <button 
              @click="removeFile(index)" 
              class="flex-shrink-0 w-5 h-5 bg-gray-100 hover:bg-red-100 text-gray-400 hover:text-red-500 rounded-full flex items-center justify-center text-xs transition-colors"
            >
              ×
            </button>
          </div>
